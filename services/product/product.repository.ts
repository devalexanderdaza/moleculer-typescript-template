...
export class InMemoryProductRepository implements IRepository<Product> {
    public async update(id: string, data: Partial<Product>): Promise<Product | null> {
    ...
    public async findByField(field: keyof Product, value: any): Promise<Product | null> {
    ...
    public async findAllByField(field: keyof Product, value: any): Promise<Product[]> {
    ...
    public async findOneAndDelete(filter: Partial<Product>): Promise<Product | null> {
        for (const [id, product] of this.products.entries()) {
            if (this.matchesFilter(product, filter)) {
    ...
    public async aggregate(pipeline: any[]): Promise<any[]> {
        ...
        for (const stage of pipeline) {
            if (stage.$match) {
                results = results.filter((product) => this.matchesFilter(product, stage.$match));
            }
            if (stage.$sort) {
